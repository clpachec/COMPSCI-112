...
void PLYObject::draw()
{
	...
	else
	{
		...
		Vector4f V, L, H, amb, dif, spe, total;

		Vector4f globalLight_ambient = { 1.0f, 1.0f, 1.0f, 1.0f }, 
			light_ambient = { 1.0f, 1.0f, 1.0f, 1.0f },
			light_diffuse = { 1.0f, 1.0f, 1.0f, 1.0f }, 
			light_specular = { 1.0f, 1.0f, 1.0f, 1.0f },
			emission = {1, 1, 1, 1};

		//Get modelviewMatrix Not sure if needed
		float matrix[16];
		...
		glGetLightfv(GL_LIGHT0, GL_AMBIENT, globalLight_ambient);
		glGetFloatv(GL_LIGHT_MODEL_AMBIENT, light_ambient);
		...
		{
			//======Global and emission======
			multVectors(total, globalLight_ambient, ambient);
			add(total, emission);
			...
			sub(L, light_pos, vertices[v]);
			normalize(L);
			diffuseNL = dotProd(normals[v], L);
			if (diffuseNL > 0){
			...
			//======Specular======
			// (max{H.N,0})n*(light specular)*(material specular)
			
			add(H, L, vertices[v]);
			normalize(H);

			float specularHN;
			specularHN = dotProd(H, normals[v]);

			if (specularHN > 0) {
				multVectors(spe, light_specular, specular);
				specularHN = pow(specularHN, shininess[0]);
				scale(spe, specularHN);
				add(total, spe);
			...
			//Put in color
			colors[v][0] = total[0] * 256;
			colors[v][1] = total[1] * 256;
			colors[v][2] = total[2] * 256;
